...
def processTok2(tokens): #ORIGEN 200
    global LC
    Tok1 = tokens[0]
    Tok2 = tokens[1].rstrip("\n")
    #Process Imperative Statement  // READE  N
    i = searchOp(Tok1) 
    if ( i == 9 or i == 10): # READ or PRINT N
        j = searchSymb(Tok2)
        if (j == -1 ):
...
for line in fp1:
    ...
    if tokCnt == 1:
        ...
        processEQU(Tok1, Tok2, Tok3)  # A EQU B
    elif tokCnt == 4:#NEXT MOVER AREG B
        Tok1 = tokens[0]
        Tok2 = tokens[1]
        Tok3 = tokens[2]
